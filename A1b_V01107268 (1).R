...
# Fit distribution for HV Patel
HV_Patel_wickets <- last_three_seasons %>%
  filter(Bowler == "HV Patel") %>%
  dplyr::select(wickets)

# Check if the resulting wickets are numeric and have more than one element
if (is.numeric(HV_Patel_wickets$wickets) && length(HV_Patel_wickets$wickets) > 1) {
  HV_Patel_wickets <- fitdist(HV_Patel_wickets$wickets, "norm")
  print(summary(HV_Patel_wickets))
} else {
  print("HV Patel's wickets are not a numeric vector of length greater than 1.")
}


# Merge performance data with salary data 
performance_salary <- left_join(ipl_rounds, salary_data, by = c("Bowler" = "Player"))
...
last_three_seasons_salary <- last_three_seasons %>%
  left_join(salary_data, by = c("Bowler" = "Player"))
